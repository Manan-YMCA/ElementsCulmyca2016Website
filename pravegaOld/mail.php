...
$fav->postDataExist('college_name');
//$fav->postDataExist('ambassador_id');
...
if(!$fav->foundErrors()) {
	...
	$ambassador_id = $sign['ambassador_id'] = $_POST['ambassador_id'];
	$day = $_POST['day'];
	$month = $_POST['month'];
	$year = $_POST['year'];
	...
	define('SUBJECT', 'Pravega student registration');
	...
	if($count == 1) {
		...
		$count=$db->countResult();
		if($count == 1) {
			$_SESSION['error'] = "Email already registered in college ambassador";
			header('location:student_registration.php');
			return false;
		}else{
			if(!empty($ambassador_id)){
				$check_sql="select * from ambassador";
				$where =" where username='".$ambassador_id."' and verification =1";
				$order = NULL;		
				$check=$db->select($check_sql, $where, $order);
				$count=$db->countResult();
				if($count != 1) {
					$_SESSION['error'] = "College ambassador id is wrong";
					header('location:student_registration.php');
					return false;
				}
			}
	...
	$check_sql="select * from student order by id desc limit 1";
	$where = NULL;
	$order = NULL;		
	$check=$db->select($check_sql, $where, $order);		
	$count=$db->countResult();
	...
	$insert_id =$db->insert('student',array('', $username, $ambassador_id, $name, $email, $password, $mobile, $city, $college_name, $dob, $know, 0, $verification_code, date('Y-m-d'))); 			
	if($insert_id) {
...
function sendEmail($to, $from, $subj, $body, $sign) {
	$message = "Welcome to Pravega 2015 <br>Your pravega username is ".$sign['username']."<br> Click on this url to confirm your registration in pravega 2015<br>
				<a href='http://www.pravega.org/activate.php?account=student&vkey=".$sign['verification_code'].'&uname='.$sign['username']."'>http://www.pravega.org/activate.php?account=student&vkey=".$sign['verification_code'].'&uname='.$sign['username']."</a>";
...
function sendEmailAdmin($to, $from, $subj, $body, $sign) {
	$message = "<h2>Student Registration</h2><br><table class='table table-striped table-bordered' align='center' style='max-width: 100%; background-color: transparent; border-collapse: collapse; border-spacing: 0; width: 100%; padding-bottom: 25px;'>
					<tr class='odd'>
					...
					<tr class='odd'>
						<td style='padding: 8px; line-height: 18px; text-align: left; vertical-align: top; border-top:1px solid #dddddd;border-left: 1px solid #dddddd; border-right: 1px solid #dddddd; border-bottom: 1px solid #dddddd; color:#555555;background-color: #f9f9f9;'>Ambassador id</td>
						<td style='padding: 8px; line-height: 18px; text-align: left; vertical-align: top; border-top:1px solid #dddddd;border-left: 1px solid #dddddd; border-right: 1px solid #dddddd; border-bottom: 1px solid #dddddd; color:#555555;background-color: #f9f9f9;'>".$sign['ambassador_id']."</td>
					</tr>
					<tr>
