<?php
	...
	$query = "DELETE FROM forgot_pass WHERE email=?";
	$stmt = mysqli_prepare($link, $query);
	mysqli_stmt_bind_param($stmt, "s", $email);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_reset($stmt);
	
	$reset_code = sha1(microtime(true).mt_rand(10000,90000));
	...
	{
		$query = "INSERT INTO forgot_pass (email, reset_code, expiry) VALUES (?,?,?)";
		$stmt = mysqli_prepare($link, $query);
		mysqli_stmt_bind_param($stmt, "ssi", $email, $reset_code, $expiry);
		mysqli_stmt_execute($stmt);
		mysqli_stmt_reset($stmt);


		// subject
		$subject = 'Password Reset at Pravega.org';

		// message
		$message = '
		<html>
		<head>
		  <title>Password Reset at Pravega.org</title>
		</head>
		<body>
			Hello, '.$first . ' '. $last .'!<br><br>
		
			You asked for your password to be reset. You can do this by following <a href="http://pravega.org/reset_form?email='. $email .'&code='.$reset_code.'">this link</a>.<br><br>
			
			If the above link doesn\'t appear for you, go to the following URL and use the reset code given:<br><br>
			URL: pravega.org/reset_form<br>
			Reset Code: '.$reset_code.'<br><br>
			
			The reset code will expire in 24 hours. Please ignore this email if you didn\'t request a password reset. <br><br>
...
<?php
	...
	if (isset($_COOKIE['email']))
	{
		...
		if ($result == $_COOKIE['hash'])
		{
			...
			$expiry_cookie = time() + 3600;
			
			$stmt = mysqli_prepare($link, $query);
			mysqli_stmt_bind_param($stmt, "is", $expiry_cookie, $_COOKIE['email']);
			mysqli_stmt_execute($stmt);
			
			setcookie("email", $_COOKIE['email'], $expiry_cookie);
			setcookie("name", $_COOKIE['name'], $expiry_cookie);
			setcookie("hash", $_COOKIE['hash'], $expiry_cookie);
		}
...
<html lang="en">
	...
	<head>
		<title>Password Reset | Pravega &ndash; the IISc Science, Tech and Cultural Fest</title>
													...
													<div id="content">
														<article class="is is-post">
															...
															<div class=row>
																...
																An email has been sent to <?php echo $to ?> with your password reset information.
																<?php
																	}
																?>
																</div>
