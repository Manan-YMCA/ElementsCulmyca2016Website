<?php
	...
	$path = "/event_data/event[subject='".$subject."']";
	$result2 = $xml->xpath($path);

	for ($i=0; $i < sizeof($result2); $i++)
	{
		$event_names[$i] = $result2[$i]->name;
		$event_links[$i] = $result2[$i]->file_name;
		$event_desc[$i] = $result2[$i]->oneline_desc;
	}
?>
<!DOCTYPE HTML>
...
<html lang="en">
	...
	<head>
		<title>Quizzes | Pravega &ndash; the IISc Science, Tech and Cultural Fest</title>
						...
						<div class="12u">
							...
							<!-- Header -->
								<div id="header">
									...
									<span>
										<a href="http://pravega.org"><img id="pravega_logo" src="images/pravega_logo_noglow<?php if (mt_rand(1, 20) == 10) echo "_sanskrit"; ?><?php if (mt_rand(1, 20) == 10) echo "_sanskrit"; ?>.png" draggable="false" style="-moz-user-select: none; -webkit-user-select: none; user-select: none; "></a>
									</span>
																...
																<div class="12u" style="padding-bottom: 25px;">
																	<?php
																		for ($i=0; $i<sizeof($event_names); $i++)
																		{
																			...
																			{
																				echo '<div class="row">';
																				echo '<section class="2u"></section>';
																			}
																				
																			echo '<div class="4u"  style="text-align:center;">';
																			echo '<h3><a href="'.$event_links[$i].'">';
																			echo $event_names[$i];
																			echo "</a></h3>";
																			echo $event_desc[$i];
																			echo "</div>";
																			...
																			if ($i % 2 == 1)
																			{
																				echo '<div class="2u"></div>';
																				echo '</div>';
																			}
																		}
																	?>
																</div>
																<hr style="margin: 1.5em 0em 0em 2.5em;">
															</div>
															
															<div class=row>
																<section class="1u"></section>
																<div class="5u hyphenate">
																	<p>Pravega bursts onto the national quiz scene with several high quality quizzes, conducted by India’s best known quizmasters. Conducted in what is arguably Bangalore’s best furnished auditorium &ndash; the JN Tata auditorium &ndash; the quizzes guarantee to enthrall, entice and educate both pros and novices alike. Bangalore being India’s Quiz capital, the competition will be tough. But again, so shall the questions. And of course, to motivate you, the prizes are BIG. </p>
																</div>
																<div class="5u hyphenate">
																	<p>With rib-ticking facts, unanticipated twists and questions that trick you houdini style, Pravega quizzes are not about just question-answer &ndash; they are high quality entertainment.</p>
																</div>
																<section class="1u"></section>
																<div class="12u" align="center" style="margin-top:10px;">
																	<h3>Be there! And bring fellow quizzophiles along!</h3>
																</div>
															</div>
														</article>
