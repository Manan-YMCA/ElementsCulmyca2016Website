<?php
	...
	if ($logged == 1)
	{
		...
		$event_num = (sizeof($event_data)/2) - 1;
		
		$query_team = "SELECT * FROM teams WHERE email='".$email."'";
		$result_team = mysqli_query($link, $query_team);
		$team_data = mysqli_fetch_array($result_team);
...
<html lang="en">
	...
	<head>
		<title>Your Events | Pravega &ndash; the IISc Science, Tech and Cultural Fest</title>
													...
													<div id="content">
														<article class="is is-post">
															<header class="style1">
																...
																<?php
																	if ($logged == 1)
																		echo $name .' &ndash; Registered Events';
																	else echo 'Registered Events';
																?>
																</h2>
																...
																<div class="12u" style="text-align:center;">
																	<?php
																		if ($logged == 1)
																		{
																			...
																			{
																	?>
																				<h2>You have registered for the following events:</h2>
																				<div style="text-align:left; display: inline-block;">
																					...
																					<?php
																						for ($i = 1; $i <= $event_num; $i++)
																						{
																							if ($event_data[$i] == 1)
																							{
																								$event_name = $event_info[$i] -> name;
																								$event_team = explode(",", $team_data[$event_name]);
																								
																								echo "<li>" . $event_info[$i] -> name . "<ol style='margin-left:40px;'>";
																								for ($j = 0; $j < sizeof($event_team); $j++)
																								{
																									echo "<li>";
																									echo $event_team[$j] . "</li>";
																								}
																								echo "</ol>";
																							}
																						}
